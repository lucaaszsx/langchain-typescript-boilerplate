...
export class AssistantKnowledge {
    private static readonly BASE_KNOWLEDGE_PATH = join(import.meta.dirname, '..', '..', 'knowledge');
    private static readonly SYSTEM_PROMPT_PATH = join(this.BASE_KNOWLEDGE_PATH, '_system.md');
    private static readonly METADATA_DB_PATH = join(import.meta.dirname, '..', '..', '.cache', 'metadata.db');
    private static readonly EMBEDDINGS_CHUNK_SIZE = 512;
    ...
    public async init(): Promise<void> {
        ...
        const { EMBEDDINGS_CHUNK_OVERLAP, EMBEDDINGS_CHUNK_SIZE, SYSTEM_PROMPT_PATH } = AssistantKnowledge;
    ...
    private getAllHashes() {
        const rows = this.metadata.prepare('SELECT source, hash FROM documents').all() as DocumentMetadata[];
    ...
    private loadDocument(source: string, basePath = AssistantKnowledge.BASE_KNOWLEDGE_PATH): Document {
